...
- No product CI test matrix; GitHub Actions allowed only for a **release publish** workflow (tag `v*` / `workflow_dispatch` builds that upload release assets) and a **redirect stub** Pages deploy of `website/` to `hembrow-innovations.github.io/odm`. The visitor site lives in `hembrow-innovations/odm-web`.
